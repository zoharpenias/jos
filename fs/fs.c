...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
    ...
    if(f->f_indirect == 0){
        ...
        if((f->f_indirect = alloc_block()) != 0) return -E_NO_DISK;         
    }
    
    uintptr_t* addr = diskaddr(f->f_indirect);
...
file_get_block(struct File *f, uint32_t filebno, char **blk)
{
       ...
       if((r = file_block_walk(f, filebno, &ppdiskbno, true))) return r;
